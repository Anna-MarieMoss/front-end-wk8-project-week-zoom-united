...
function Quotes({ id }) {
  const [newQuote, setNewQuote] = useState("Every day may not be good but there is something good in every day!");
  ...
  useEffect(() => {
    async function getRandomQuote() {
      ...
      const {payload } = data;
      // const dbQuote = { quote: data.quote };
      console.log(payload[0].quote);
  ...
  return (
    <div>
      <h2 className="quote"><u>Quote</u><br></br> {JSON.stringify(newQuote)}<br></br>
      </h2>
      {/* <button
          onClick={() => {
            getRandomQuote();
